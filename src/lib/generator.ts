      ...
      if (member.decorators) {
        ...
        if (mlMethod) {
          ...
          if (mlMethod.parameters && mlMethod.parameters[0]) {
            ...
            method.method = ee.value === 'PUT' ? mu.METHOD.PUT : mu.METHOD.POST
          }
          if (!method.method) {
            method.method = mu.METHOD.POST
          }
          return method
        }
      }
...
export function generateAssetModel(serviceSpecs: m.MLServices, baseUri: string, assetModel: MarkScript.AssetModel, pkgDir?: string) {
...
throw error;
  ...
  return value
        ...
        onEvent: function(event) {
          let states: MarkScript.TRIGGER_STATE[]
          if (!event.states) {
            states = [MarkScript.TRIGGER_STATE.CREATE, MarkScript.TRIGGER_STATE.MODIFY]
          } else if (Array.isArray(event.states)) {
            states = <Array<MarkScript.TRIGGER_STATE>>event.states
          } else {
            states = [<MarkScript.TRIGGER_STATE>event.states]
          }
